...
@property (nonatomic, strong) XDXSortFrameHandler *sortHandler;
...
- (XDXSortFrameHandler *)sortHandler {
    if (!_sortHandler) {
        _sortHandler = [[XDXSortFrameHandler alloc] init];
        _sortHandler.delegate = self;
    }
    return _sortHandler;
}

- (XDXPreviewView *)previewView {
    if (!_previewView) {
        _previewView = [[XDXPreviewView alloc] initWithFrame:self.view.frame];
    }
    return _previewView;
}
